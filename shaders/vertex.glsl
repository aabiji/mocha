...
void main()
{
    ...
    for (int i = 0; i < 4; i++) {
        if (boneIds[i] == -1) break; // No more bones
        boneTransform += boneTransforms[boneIds[i]] * boneWeights[i];
    }
    vec4 position = boneTransform * vec4(_position, 1.0);
    vec4 normal = boneTransform * vec4(_normal, 0.0);
    vec4 tangent = boneTransform * vec4(_tangent, 0.0);
    ...
    fragOut.fragmentPos = TBN * vec3(model * position);
